...
uint16 tcp_checksum(uint32 saddr, uint32 daddr, uint16 protocol, uint16 tcpLen, uint8* tcpPackage)
{
    if (NULL == tcpPackage || 0 == tcpLen)
    {
        return 0x0000;
    }

    size_t pseudoHeadLen = 12;
    uint8* checkData = (uint8*)malloc(pseudoHeadLen + tcpLen);
    if (NULL == checkData)
    {
        return 0x0000;
    }

    uint8* data = checkData;
    ...
    *(uint16*)data = protocol;
    data += sizeof(uint16);

    *(uint16*)data = tcpLen;
    data += sizeof(uint16);

    memcpy(data, tcpPackage, tcpLen);

    return internet_checksum(checkData, pseudoHeadLen + tcpLen);
}
